...
class FakeInput {
  ...
  update(key, keyDown){
    ...
    if (keyDown)
      ...
      this.simulator.input.up(key);
  }

  set up(v)    { this.update('ArrowUp'   , v); }
  set right(v) { this.update('ArrowRight', v); }
  set down(v)  { this.update('ArrowDown' , v); }
  set left(v)  { this.update('ArrowLeft' , v); }
  set jump(v)  { this.update('Space'     , v); }
}
...
class Simulator {
  ...
  constructor(template, customMappings){
    ...
    const width = lines.reduce((v, line) => Math.max(line.length, v), 0);

    this.world.clearWorld(width, height, 1);
    for (let y = 0; y < height; y++){
      for (let x = 0; x < width; x++){
        const ch = mappings[lines[y].charAt(x) || ' '];
        if (typeof ch === 'undefined')
          throw new Error(`Unknown test tile: ${lines[y].charAt(x)}`);
        let placeTile = false;
        let properties;
        switch (ch){
          case 'blank'      : break;
          ...
          case 'player':
            if (this.spawn)
              throw new Error('Cannot set player spawn twice');
            this.spawn = {x, y};
            break;
          default:
